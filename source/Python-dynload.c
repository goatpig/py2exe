...
int _load_python_FromFile(char *dllname)
{
	...
	if (hmod == NULL)
	{
		wchar_t *dllnamew = (wchar_t*)malloc((strlen(dllname)+1)*sizeof(wchar_t));
		MultiByteToWideChar(CP_UTF8, 0, dllname, -1, dllnamew, strlen(dllname)+1);
		hmod = LoadLibraryW(dllnamew);
		free(dllnamew);
	}
	if (hmod == NULL) {
		return 0;
